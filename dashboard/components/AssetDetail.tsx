...
function Stat({
  ...
  return (
    <div className="panel min-w-[120px] px-3 py-2">
      ...
      <div className="font-mono text-lg font-bold leading-tight" style={{ color: accent }}>
        {value}
      </div>
      {hint && (
        <div className="mt-0.5 font-mono text-[9px] uppercase text-ink-dim">{hint}</div>
      )}
    </div>
  );
}
...
export default function AssetDetail({ asset }: { asset: AssetPayload }) {
  ...
  const monthRet = startClose && endClose ? endClose / startClose - 1 : null;
    ...
    <section className="mx-auto max-w-7xl px-4 py-6">
      ...
      <header className="mb-5 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          />
          {asset.stats.sharpe_p50 !== undefined && (
            <Stat
        ...
        <aside className="space-y-3">
          ...
          <div className="panel p-4">
            ...
            <div className="space-y-2">
              {asset.current_gmm.probs.map((p, i) => {
                ...
                return (
                  <div key={i}>
                    <div className="flex items-center justify-between font-mono text-[11px]">
                      <span style={{ color: c }}>{["Bull", "Neutral", "Bear"][i]}</span>
                      <span className="text-ink-muted">{fmtPct(p)}</span>
                    </div>
                    <div className="mt-0.5 h-1.5 w-full rounded bg-bg-ring">
                      <div
            ...
            <p className="mt-3 text-[10px] leading-relaxed text-ink-dim">
              3-state Gaussian HMM fit on (log-return, EWMA-vol). Unsupervised — states ranked by
              ascending variance. Agreement with the rule baseline = high confidence.
            </p>
          </div>
          ...
          {/* Backtest stats — only shown for universe assets with computed stats */}
          {asset.stats.sharpe_p50 !== undefined && (
            <div className="panel p-4 text-xs">
              ...
              <p className="mt-3 text-[10px] leading-relaxed text-ink-dim">
                TVTP-MSAR position maps state 0 (low-vol) → +100% and state 1 (high-vol) → −30%
                defense.
              </p>
            </div>
          )}
